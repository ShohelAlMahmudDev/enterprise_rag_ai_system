...
# ALLOWED_EXTENSIONS = {'.pdf', '.docx', '.txt', '.xlsx', '.pptx'} | SUPPORTED_IMAGE_EXTENSIONS
...
#         raise ValueError(f"Unsupported file type: {ext or 'unknown'}. Allowed: {sorted(ALLOWED_EXTENSIONS)}")
#     return ext
...
#         raise ValueError(f'File too large. Maximum supported size is {settings.MAX_FILE_SIZE_MB} MB.')


# def safe_join_upload(filename: str) -> str:
#     clean = os.path.basename(filename).replace(' ', '_')
#     return str(Path(settings.UPLOAD_DIR) / clean)
...
# def safe_join_upload(filename: str, unique: bool = False) -> str:
