...
#         self.multimodal = MultimodalIngestionService() if settings.ENABLE_MULTIMODAL_INGESTION else None

#     async def create_document(self, file: UploadFile, logical_name: str | None = None, notes: str | None = None):
#         resolved_name = (logical_name or '').strip() or Path(file.filename or 'document').stem
#         with SessionLocal() as db:
#             document = Document(logical_name=resolved_name, current_version=1, is_deleted=False)
#             db.add(document)
#             db.flush()
#             version = await self._create_version_record(db, document, file, 1, notes)
#             db.commit()
#             db.refresh(document)
#             db.refresh(version)
#             return self._map_document(document, version), self._map_version(version)

#     async def create_new_version(self, document_id: str, file: UploadFile, notes: str | None = None):
#         with SessionLocal() as db:
...
#             version = await self._create_version_record(db, document, file, new_version_number, notes)
...
#         with SessionLocal() as db:
#             stmt = select(Document).options(selectinload(Document.versions)).order_by(Document.updated_at.desc())
...
#         with SessionLocal() as db:
#             stmt = select(DocumentVersion).where(DocumentVersion.document_id == document_id).order_by(DocumentVersion.version.desc())
...
#     async def _create_version_record(self, db, document: Document, file: UploadFile, version_number: int, notes: str | None):
...
#                 raise ValueError('Image upload is enabled only when multimodal ingestion is configured.')
...
#             language = self.embedder.detect_language(text)
...
#         )
#         db.add(version)
#         logger.info('Indexed %s chunks for document %s version %s', chunk_count, document.id, version_number)
#         return version
...
#     def _map_document(self, doc: Document, version: DocumentVersion | None = None) -> DocumentOut:
