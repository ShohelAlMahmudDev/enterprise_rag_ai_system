...
class AgentService:
    ...
    def _generate_with_context(self, results: list[dict[str, Any]], question: str) -> tuple[str, str]:
        ...
        grounded_question = self._build_grounded_generation_prompt(question)

        try:
            answer = self.llm.generate(results, grounded_question)
        except Exception as exc:
            logger.exception("LLM generation failed: %s", exc)
            answer = (
                "I found relevant information, but I could not generate a final answer "
    ...
    def _build_retrieval_query(self, question: str, history: list[dict[str, Any]]) -> str:
        """
        Keep exact lookup queries clean.
        Add history only when the user question looks conversational/ambiguous.
        """
        if self._is_structured_lookup_query(question):
            return question
    ...
    def _build_grounded_generation_prompt(self, question: str) -> str:
        return (
            "Answer using only the retrieved context. "
            "For technical tables, field names, codes, and numeric mappings, prefer exact matches from the sources. "
            "Do not guess or interpolate neighboring values. "
            f"\n\nUser question: {question}"
        )

    def _is_structured_lookup_query(self, question: str) -> bool:
        q = (question or "").strip().lower()
        if not q:
            return False

        if len(q) <= 80 and any(ch.isdigit() for ch in q):
            return True
